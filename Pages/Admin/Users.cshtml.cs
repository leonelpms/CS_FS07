        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            foreach (var user in Users)
            {
                ...
                if (toAdd.Any())
                {
                    ...
                    if (!result.Succeeded)
                    {
                        StatusMessage = $"Error al agregar roles a {user.Email}: {string.Join(", ", result.Errors.Select(e => e.Description))}";
                ...
                if (toRemove.Any())
                {
                    ...
                    if (!result.Succeeded)
                    {
                        StatusMessage = $"Error al quitar roles a {user.Email}: {string.Join(", ", result.Errors.Select(e => e.Description))}";
                        return RedirectToPage();
                    }
                    anyChanged = true;
                }
            ...
            StatusMessage = anyChanged ? "Roles actualizados correctamente." : "No hubo cambios en los roles.";
            return RedirectToPage();
        }
    }
}